...
import { useAddTypeWork, useUpdateTypeWorkPhotos, useUpdateTypeWorkStatus } from "../../../core/network/queries/typeWork/mutations";
...
export function AddTypeOfWorkDialog({
  ...
  const handleAddTypeOfWork = () => {
    addTypeWork({name: typeWorkName, status_list: []  } as CreateTypeWorkDTO, 
      {onSuccess(data) {
          updateTypeWorkPhotos({updateFlags: photoFlags, typeWorkFlag: data.flag!} as UpdateParams);
          updateTypeWorkStatus({updateFlags: workStatusFlags, typeWorkFlag: data.flag!} as UpdateParams);
      },}
  );
    handleCloseDialog();
  };
            ...
            <TableBody>
              {typePhotos?.map((value) => (
                <TableRow key={value.flag}>
                  <TableCell>
                    <Checkbox 
                      onChange={(e)=> handlePhotoCheck(value.flag, e.target.value )}
                    />
                  </TableCell>
            ...
            <TableBody>
              {workStatus?.map((value, index) => (
                <TableRow key={value.flag}>
                  <TableCell>
                    <Checkbox 
                      onChange={(e)=> handleWorkStatusCheck(value.flag, e.target.value)}
                    />
                  </TableCell>
                  <TableCell>{value.name}</TableCell>
                </TableRow>
