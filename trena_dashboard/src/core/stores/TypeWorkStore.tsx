...
export class TypeWorkStore extends BaseStore {
  ...
  search(query?: string) {
    if (query) {
      this.typeWorksList = this.fullTypeWorksList.filter((item) =>
  ...
  async updateTypeWorkWorkStatus(workStatuses: number[], typeWorkFlag: number) {
    this.baseCall(async () => {
      await TypeWorkService.updateTypeWorkWorkStatus(
        workStatuses,
        typeWorkFlag
      );
      runInAction(() => {
  ...
  async loadSelectedTypeWorkWorkStatus() {
    if (this.selectedTypeWork) {
      this.baseCall(async () => {
        const workStatus = await TypeWorkService.loadTypeWorkWorkStatus(
          this.selectedTypeWork!.flag!
        );
        runInAction(() => {
  ...
  async loadSelectedTypeWorkTypePhotos() {
    if (this.selectedTypeWork) {
      this.baseCall(async () => {
        const typePhotos = await TypeWorkService.loadTypeWorkTypePhotos(
          this.selectedTypeWork!.flag!
        );
        runInAction(() => {
