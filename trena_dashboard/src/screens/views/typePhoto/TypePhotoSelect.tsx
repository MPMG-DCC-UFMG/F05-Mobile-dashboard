...
export default class TypePhotoSelect extends BaseCRUDView<
  TypePhotoSelectProps,
  TypePhotoSelectState
> {
  constructor(props: TypePhotoSelectProps) {
    ...
    if (props.defaultSelected) {
      this.state = {
        selectedOptions: new Set<string>(
          props.defaultSelected.map((value) => value.toString())
        ),
      };
    }
  }
  ...
  handleWorkChange = (selected: Set<string>) => {
    if (this.props.onChangeSelectedOptions) {
      let mArray = Array.from(selected).map(
